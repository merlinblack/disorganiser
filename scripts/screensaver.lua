...
function ScreenSaver:setTime(secs) self.time = secs end
...
function ScreenSaver:setPicture(filename)
	...
	-- Center
	if dest[3] < width then dest[1] = width // 2 - dest[3] // 2 end
	if dest[4] < height then dest[2] = height // 2 - dest[4] // 2 end
...
function ScreenSaver:mouseClick(time, x, y, button) self.previousScreen:activate(true) end
...
function ScreenSaver:swipe() wakeTask 'screensaver' end
...
function screenSaveTask()
	...
	while not screenSaver.stop do
		...
		if not picture then
			screenSaver.pictureIter = dirList(screenSaver.directory .. '*.{jpg,JPG,jpeg,png}')
			picture = screenSaver.pictureIter()
		end
