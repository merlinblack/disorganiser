...
function Suspend:mouseClick(time, x, y, button) self.shouldWake = true end
...
function Suspend:setDisplayPower(value)
	...
	if app.displayName == nil then app.displayName = 'HDMI-1' end

	local program = 'xrandr'
	if app.displayType == 'wayland' then program = 'wlr-randr' end
