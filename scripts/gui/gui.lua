...
function addButton(self, rect, captionText, func, textColor, frameColor, backgroundColor, renderList)
    if not renderList then
        renderList = self.renderList
    end

    if not self.font then
        return error('No font set for adding button to: ' .. self.__type)
    end
