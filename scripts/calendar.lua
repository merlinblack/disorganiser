...
function Calendar:init()
	...
	for i, v in pairs(self.text.weekdaysShort) do
		...
		local hovertext = HoverText(
			{ x, 74, texture.width, texture.height },
			self.text.weekdays[i],
			Color 'fff',
			nil,
			nil,
			Color 'ff0000ff'
	...
	self:addButton(
		{ app.width - 120, buttonTop, 115, 50 },
		'Geri',
...
function Calendar:calcAppointments()
	...
	for _, v in pairs(appointments) do
		...
		if n.month == -1 then
			n.month = self.month
			if n.day < self.currentDay and self.currentMonth == self.month then n.month = (n.month % 12) + 1 end
		end
		if n.year == -1 then
			n.year = self.year
			if n.month < self.month then n.year = n.year + 1 end
		end
...
function Calendar:buildTextTextures()
	...
	for _, v in pairs(self.text) do
		if type(v) == 'string' then self.textures[v] = Texture(self.font2, v, Color 'fff') end
	end
	for i = 1, 31 do
...
function Calendar:buildAppointments()
	...
	for _, appointment in pairs(self.appointments) do
		if appointment.timestamp > start and appointment.timestamp < limit then
			...
			if not self.portrait then y = y + self.font3.lineHeight + 1 end

			local texture <close> = Texture(
				self.font3,
				string.format(
					'%s %d%s %s',
					text.weekdays[wday],
...
function Calendar:getColorForDate(year, month, day)
	...
	for _, appointment in pairs(self.appointments) do
		if timestamp == appointment.timestamp then return Color(appointment.color) end
	end
...
function Calendar:isLeapYear() return self.year % 4 == 0 and (self.year % 100 ~= 0 or self.year % 400 == 0) end

function Calendar:swipe(direction)
	if direction == Swipe.Right then self:previous() end
	if direction == Swipe.Left then self:next() end
...
addTask(function()
	while wait(60000) ~= 'killed' do
		if calendar.month == calendar.currentMonth and calendar.year == calendar.currentYear then
			if calendar.currentDay ~= os.date('*t').day then calendar:today() end
		end
	end
end, 'calendar')
