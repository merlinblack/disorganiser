...
function ConfirmDialog:activate()
	if self.prevRenderList == nil then self.prevRenderList = app.overlay end
...
function ConfirmDialog:build()
	...
	self:addButton(
		{ self.dialogLeft, btnY, self.buttonWidth, btnHeight },
		'Tamam',
		function() self.result = 'ok' end,
		Color 'fff',
		self.frameColor,
		self.backColor
	)
	self:addButton(
		{ self.dialogRight - self.buttonWidth, btnY, self.buttonWidth, btnHeight },
		'Iptal',
		function() self.result = 'cancel' end,
		Color 'fff',
		self.frameColor,
		self.backColor
	)
...
function ConfirmDialog:run(message)
	...
	if self.previousScreen then self.previousScreen:activate() end
