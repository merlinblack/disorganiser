...
function Unlock:build()
	...
	for row = 1, secret.height do
		for col = 1, secret.width do
			local btn = {
				left + (col - 1) * (buttonWidth + buttonGap),
				top + (row - 1) * (buttonHeight + buttonGap),
				buttonWidth,
				buttonHeight,
			}
			self:addButton(
				btn,
				secret.buttons[buttonIndex],
				self:getBtnPressedClosure(secret.buttons[buttonIndex]),
				textcolor,
				nil,
				backcolor
			)
...
function Unlock:btnPressed(value)
	...
	if self.code == secret.pass then Unlock:unlock 'media/special/' end
	if self.code == secret.pass2 then Unlock:unlock 'media/special2/' end
end
