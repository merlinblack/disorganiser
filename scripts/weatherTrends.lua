...
function WeatherTrends:build()
	...
	self:addButton(btn, 'Grafikler', function()
		weatherGraphs.previousScreen = self.previousScreen
		weatherGraphs:activate(true)
	end, textcolor, nil, backcolor)

	function updateTask()
		while true do
			if self:isActive() then self:buildDataTable() end
			wait(60000)
		end
	end
...
function WeatherTrends:buildDataTable(force)
	...
	if weather.valid == false or weatherTrendsData == nil then return end
