...
bool Application::initSDL(bool fullscreen)
{
  ...
  if (failed) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Could not init SDL: %s",
                 sdl->getLastErrorMessage().c_str());
    return true;
  }

  failed = sdl->createWindow("Disorganiser", SDL_WINDOWPOS_UNDEFINED,
  ...
  SDL_GetRendererOutputSize(const_cast<SDL_Renderer*>(sdl->getRenderer()),
                            &width, &height);
...
void Application::loadConfig(std::string configPath)
{
  ...
  if (configPath.empty()) {
    if (!getenv("XDG_CONFIG_PATH")) {
      ss << getenv("HOME");
      ss << "/.config";
    }
    else {
      ss << getenv("XDG_CONFIG_PATH");
    }
    ss << "/disorganiser/config.lua";
  }
  else {
    ss << configPath;
  }
  ...
  if (mouse.isFunction()) {
    switch (event.type) {
      ...
      case SDL_MOUSEMOTION:
        mouse(event.type, event.button.x, event.button.y, event.button.button,
              event.button.state, event.button.clicks, event.button.which);
        break;
    }
  }
}
...
void Application::handleTouch(const SDL_Event& event)
{
  ...
  if (touch.isFunction()) {
    touch(event.type, event.tfinger.x, event.tfinger.y, event.tfinger.dx,
          event.tfinger.dy);
  }
}
...
void Application::dispatchEvent(const SDL_Event& event)
{
  switch (event.type) {
    case SDL_QUIT:
      ...
      break;
    /*
    TODO: figure out what window event we should trigger rendering on, after
    being minimized. case SDL_WINDOWEVENT_SHOWN: shouldRender = true; break;
    */
    case SDL_TEXTINPUT:
