...
Rectangle::Rectangle(TexturePtr texture, int x, int y)
    : texture(texture), source({0, 0, 0, 0}), clip({0, 0, 0, 0})
{
  destination = {x, y, 0, 0};
}

Rectangle::Rectangle(TexturePtr texture, SDL_Rect dest, SDL_Rect src)
    : texture(texture), destination(dest), source(src), clip({0, 0, 0, 0})
{
...
{
  SDL_SetRenderDrawColor(const_cast<SDL_Renderer*>(renderer), color.r, color.g,
                         color.b, color.a);
  if (fill) {
