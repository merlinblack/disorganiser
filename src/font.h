...
class Font {
 ...
 public:
  ...
  ~Font()
  {
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "~Font %lx\n",
                 (unsigned long)this);
  }
  ...
  TexturePtr renderTextQuick(const SDL_Renderer* renderer,
                             const std::string& text,
                             SDL_Color color);
  TexturePtr renderTextNice(const SDL_Renderer* renderer,
                            const std::string& text,
                            SDL_Color color);
};
